...
# collection = db['plantpot']
#建立資料夾
if not os.path.exists('./plantpots'):
    os.mkdir('./plantpots')
...
for i in range(1,2):#頁數
    payload = {'page':i}
    res = requests.get(url, headers=headers, params = payload)
    # print(res.url) ##當前爬取的網址
    soup = BeautifulSoup(res.text, 'lxml')
    soupArticle = soup.select('div[class="card px-0 px-md-4"]')
    for idex, article in enumerate(soupArticle):
...
#         print(articleUrl)
        ...
        itemsInform = {"_id":idex+1}  #設定資料表ID
        ...
        # except pymongo.errors.DulicateKeyError as err:
        #     print(err)
...
#             imagePath = './plantpots/{}_{}.{}'.format(titleItem, idx, imageUrl.split('.')[-1])
...
# 
    ...
    print("="*20)
#存入JSON
with open('./jason_ikea3.json', 'w', encoding='utf-8') as jsonfile:
    jsonfile.write(json.dumps(resultJson))
#     json.dump(json.dumps(resultJson), jsonfile)
# print(resultJson)